import os
import io
import numpy as np
import cv2
import onnxruntime as ort
from flask import Flask, request, jsonify, send_from_directory
from flask_cors import CORS
from PIL import Image

# ── App setup ────────────────────────────────────────────────────────────────
app = Flask(__name__, static_folder="frontend/dist", static_url_path="")
CORS(app, origins=["http://localhost:5173", "http://127.0.0.1:5173"])

# ── Model setup ───────────────────────────────────────────────────────────────
MODEL_PATH = os.path.expanduser("~/.insightface/models/buffalo_l/w600k_r50.onnx")
EMBEDDINGS_DIR = os.path.join(os.path.dirname(__file__), "embeddings")
DRIFT_THRESHOLD = 0.65

print(f"[startup] Loading ONNX model from: {MODEL_PATH}")
session = ort.InferenceSession(MODEL_PATH, providers=["CPUExecutionProvider"])
INPUT_NAME = session.get_inputs()[0].name
print(f"[startup] Model loaded. Input name: {INPUT_NAME}")

# ── Helpers ───────────────────────────────────────────────────────────────────

def preprocess_image(file_bytes: bytes) -> np.ndarray:
    """Convert raw image bytes → normalised CHW float32 tensor ready for ArcFace."""
    pil_img = Image.open(io.BytesIO(file_bytes)).convert("RGB")
    img = np.array(pil_img)               # RGB, HWC, uint8
    img = cv2.resize(img, (112, 112))     # resize to ArcFace input
    img = img.astype(np.float32)
    img = (img - 127.5) / 128.0          # normalise to [-1, 1]
    img = img.transpose(2, 0, 1)         # HWC → CHW
    img = np.expand_dims(img, axis=0)    # add batch dim → (1, 3, 112, 112)
    return img


def get_embedding(file_bytes: bytes) -> np.ndarray:
    """Run inference and return L2-normalised 512-dim embedding."""
    tensor = preprocess_image(file_bytes)
    feat = session.run(None, {INPUT_NAME: tensor})[0].flatten()
    feat = feat / (np.linalg.norm(feat) + 1e-8)
    return feat


def verdict_text(drift: float) -> str:
    if drift > DRIFT_THRESHOLD:
        return "Re-enrollment Recommended"
    if drift > 0.3:
        return "Moderate Drift Detected"
    return "Identity Stable"


# ── Routes ────────────────────────────────────────────────────────────────────

@app.route("/")
def index():
    """Serve built React frontend."""
    try:
        return send_from_directory(app.static_folder, "index.html")
    except Exception:
        return jsonify({"message": "Backend running. Start React dev server on port 5173."})


@app.route("/compare", methods=["POST"])
def compare():
    """
    POST /compare
    Body: multipart/form-data with fields 'image1' and 'image2'
    Returns: JSON { similarity, drift, flagged, verdict, drift_zone }
    """
    if "image1" not in request.files or "image2" not in request.files:
        return jsonify({"error": "Both 'image1' and 'image2' are required."}), 400

    try:
        bytes1 = request.files["image1"].read()
        bytes2 = request.files["image2"].read()

        emb1 = get_embedding(bytes1)
        emb2 = get_embedding(bytes2)

        similarity = float(np.dot(emb1, emb2))
        # Clamp to [-1, 1] in case of floating-point drift
        similarity = max(-1.0, min(1.0, similarity))
        drift = round(1.0 - similarity, 4)
        similarity = round(similarity, 4)
        flagged = drift > DRIFT_THRESHOLD

        if drift < 0.3:
            drift_zone = "stable"
        elif drift < DRIFT_THRESHOLD:
            drift_zone = "moderate"
        else:
            drift_zone = "critical"

        return jsonify(
            {
                "similarity": similarity,
                "drift": drift,
                "flagged": flagged,
                "verdict": verdict_text(drift),
                "drift_zone": drift_zone,
                "threshold": DRIFT_THRESHOLD,
            }
        )
    except Exception as e:
        return jsonify({"error": f"Inference failed: {str(e)}"}), 500


@app.route("/results", methods=["GET"])
def results():
    """
    GET /results
    Loads precomputed benchmark .npy files and returns summary stats.
    """
    DATASET_META = {
        "lfw":       {"label": "LFW",      "accuracy": 99.67},
        "calfw":     {"label": "CALFW",     "accuracy": 94.02},
        "agedb_30":  {"label": "AgeDB-30",  "accuracy": 92.68},
        "cplfw":     {"label": "CPLFW",     "accuracy": 88.73},
    }

    output = []
    for name, meta in DATASET_META.items():
        npy_path = os.path.join(EMBEDDINGS_DIR, f"{name}_results.npy")
        if not os.path.exists(npy_path):
            output.append({
                "dataset": meta["label"],
                "accuracy": meta["accuracy"],
                "stability_index": None,
                "avg_drift": None,
                "reenrollment_pct": None,
                "total_pairs": None,
            })
            continue

        data = np.load(npy_path, allow_pickle=True)

        # Accuracy (correct classifications)
        correct = sum(
            1 for r in data
            if (r["drift"] < DRIFT_THRESHOLD) == (r["label"] == 1)
        )
        accuracy = round(correct / len(data) * 100, 2)

        # Stability index — average similarity of genuine (same-person) pairs
        same = [r for r in data if r["label"] == 1]
        avg_sim = float(np.mean([r["similarity"] for r in same])) if same else 0.0
        stability_index = round(avg_sim, 4)

        # Average drift across all pairs
        avg_drift = round(float(np.mean([r["drift"] for r in data])), 4)

        # Re-enrollment needed among genuine pairs
        flagged_same = sum(1 for r in same if r["drift"] > DRIFT_THRESHOLD)
        reenrollment_pct = round(flagged_same / len(same) * 100, 1) if same else 0.0

        output.append(
            {
                "dataset": meta["label"],
                "accuracy": accuracy,
                "stability_index": stability_index,
                "avg_drift": avg_drift,
                "reenrollment_pct": reenrollment_pct,
                "total_pairs": len(data),
            }
        )

    return jsonify({"results": output, "threshold": DRIFT_THRESHOLD})


@app.route("/health", methods=["GET"])
def health():
    return jsonify({"status": "ok", "model": "w600k_r50 (ArcFace)", "threshold": DRIFT_THRESHOLD})


# ── Entry point ───────────────────────────────────────────────────────────────
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=True)
