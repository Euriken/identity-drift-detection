...
export default function Navbar() {
  ...
  const [backendOnline, setBackendOnline] = useState(null); // null=checking, true=ok, false=down
  ...
  useEffect(() => {
    ...
    check();
    const interval = setInterval(check, 15000); // re-check every 15 s
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const current = NAV_ITEMS.reduce((acc, n) => {
        const el = document.getElementById(n.id);
        if (el && el.getBoundingClientRect().top <= 120) return n.id;
        return acc;
      }, 'hero');
      setActive(current);
    };
  ...
  // ── Status dot ────────────────────────────────────────────────────────────
  const dotColor   = backendOnline === null ? '#fbbf24' : backendOnline ? '#34d399' : '#f43f5e';
  const dotLabel   = backendOnline === null ? 'Checking…' : backendOnline ? 'Backend online' : 'Backend offline';
  const dotPulse   = backendOnline === true;
    ...
    <nav style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => scrollTo('hero')}>
          ...
          <div title={dotLabel} style={{ display: 'flex', alignItems: 'center', gap: 5, marginLeft: 4 }}>
            <div style={{
              width: 8, height: 8, borderRadius: '50%',
              background: dotColor,
              boxShadow: dotPulse ? `0 0 6px ${dotColor}` : 'none',
              animation: dotPulse ? 'pulse-glow 2s ease-in-out infinite' : 'none',
              transition: 'background 0.4s ease',
            }} />
            <span style={{ fontSize: '0.65rem', color: dotColor, fontWeight: 600, letterSpacing: '0.04em', display: scrolled ? 'inline' : 'none' }}>
              {backendOnline === false ? 'OFFLINE' : backendOnline ? 'ONLINE' : '…'}
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }} className="desktop-nav">
          ...
          <button onClick={() => scrollTo('demo')} className="btn-primary" style={{ padding: '7px 18px', fontSize: '0.83rem', marginLeft: 8 }}>
            Try Demo →
          </button>
        </div>
